...
export default async function handler(req, res) {
  ...
  try {
    const { org_name, org_link, org_role, email } = req.body;

    await resend.emails.send({
      from: 'Mālama Map <noreply@malamamap.org>',
      to: 'dahlthurstonstar@gmail.com',
      replyTo: email,
      subject: 'New verification request — ' + org_name,
      html: `
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#f4f6f4;padding:32px 24px;">
          <div style="background:linear-gradient(135deg,#1a3d2b,#3a7d5c);border-radius:16px;padding:24px;text-align:center;margin-bottom:24px;">
            ...
            <p style="color:rgba(255,255,255,0.65);font-size:13px;margin:8px 0 0;letter-spacing:0.08em;">NEW VERIFICATION REQUEST</p>
          </div>
          <div style="background:white;border-radius:14px;padding:24px;margin-bottom:16px;border:1px solid rgba(0,0,0,0.06);">
            <div style="background:#edf7f1;border-left:4px solid #3a7d5c;border-radius:0 10px 10px 0;padding:14px 16px;margin-bottom:20px;">
              <p style="font-weight:700;color:#1a1a1a;margin:0;font-size:15px;">${org_name}</p>
            </div>
            <table style="width:100%;border-collapse:collapse;font-size:14px;">
              <tr><td style="padding:8px 0;color:#8a827a;width:120px;">Email</td><td style="padding:8px 0;"><a href="mailto:${email}" style="color:#3a7d5c;font-weight:700;">${email}</a></td></tr>
              <tr><td style="padding:8px 0;color:#8a827a;">Role</td><td style="padding:8px 0;font-weight:700;color:#1a1a1a;">${org_role || '—'}</td></tr>
              <tr><td style="padding:8px 0;color:#8a827a;">Link</td><td style="padding:8px 0;color:#1a1a1a;">${org_link || '—'}</td></tr>
            </table>
